...
class AIOHandler:
    ...
    def _emit_values(self, raw: np.ndarray, ods_status: int, cushion_sum: float):
        ...
        # ✅ Toujours émettre frame_update avec les valeurs capteurs
        ...
        # Garder ods_update séparément pour le statut visuel
        if ods_status == 0:
            ...
            for line in lines:
                ...
                try:
                    ...
                    if ods_status == 0:
                        ...
                        if self.ods_state == 0:
                            # Premier contact : démarrer le timer ODS
                            self.ods_state = 1
                            self.ods_detection_time = time.time()

                        # ✅ Toujours émettre les valeurs capteurs (affichage live)
                        self.socketio.emit("frame_update", {
                            "person_id": self.current_person,
                            "frame_idx": self.frame_count,
                            "sensors": raw.flatten().tolist()[:10],
                        ...
                        if elapsed >= self.delay_duration and self.is_predicting:
                            ...
                            print(f"  ▶ Frame {self.predict_frame_count:03d}/{self.PREDICT_TARGET_FRAMES} | {self.current_person}")
                            for model_name, pred in preds.items():
                                if pred is not None:
                            ...
                            if self.predict_frame_count >= self.PREDICT_TARGET_FRAMES:
                                ...
                                # Fallback si buffer vide
                                if not results:
                                    try:
                                        for model_name, last_val in (preds or {}).items():
                                            if last_val is not None:
                                                results.append({'model': model_name, 'predicted': round(float(last_val), 1)})
                                        if results:
                                            print('  ⚠️ Fallback: using last-frame predictions')
                                    except Exception as e:
                                        print('  ⚠️ Error building fallback results:', e)
                                ...
                                self.socketio.emit('predict_result', {
                                    'results': results,
                    ...
                    print(f"Error parsing line: {line}, error: {line_error}")
                    continue
